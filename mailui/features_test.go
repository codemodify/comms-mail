...
func TestUnifiedInboxAggregates(t *testing.T) {
	...
	ada := s.ListMessages(mailcore.FolderAdaInbox)
	work := s.ListMessages(mailcore.FolderWorkInbox)
	uni := s.ListMessages(mailcore.FolderUnifiedInbox)
	if len(uni) != len(ada)+len(work) {
		t.Fatalf("unified %d want %d+%d", len(uni), len(ada), len(work))
	}
	unread := s.ListMessages(mailcore.FolderUnifiedUnread)
	if len(unread) == 0 {
		t.Fatal("unified unread")
	}
	star := s.ListMessages(mailcore.FolderUnifiedStarred)
	if len(star) == 0 {
		t.Fatal("unified starred")
	}
	imp := s.ListMessages(mailcore.TagFolderID("Important"))
	if len(imp) == 0 {
		t.Fatal("tag Important")
	}
	f, ok := s.GetFolder(mailcore.FolderUnifiedInbox)
	if !ok || !f.Virtual {
		t.Fatal("virtual get")
	}
	if s.Unread(mailcore.FolderUnifiedInbox) < 1 {
...
func TestIdentitiesAndTags(t *testing.T) {
	...
	ada := s.Identities(mailcore.AcctAda)
	if len(ada) < 2 {
	...
	if _, err := s.PutTag(mailcore.Tag{Name: "Later", Color: "#111111"}); err != nil {
		t.Fatal(err)
	}
...
func TestFilterRulesApply(t *testing.T) {
	...
	found := false
	for _, m := range s.ListMessages(mailcore.FolderAdaInbox) {
		if strings.Contains(m.Subject, "Invoice") && mailcore.HasTag(m.Tags, "Work") {
			found = true
			break
		}
	}
...
func TestRPCNewMethods(t *testing.T) {
	...
	uni, err := cli.ListMessages(mailcore.FolderUnifiedInbox, mailcore.Filter{})
	...
	res, err := cli.Sync(mailcore.AcctAda)
	if err != nil {
		t.Fatal(err)
	}
	if res.New < 0 {
...
func TestMIMEParseAndSanitize(t *testing.T) {
	raw := []byte("From: Ada <ada@example.com>\r\n" +
		...
		"hello body\r\n")
	m, err := mailcore.ParseRFC822(raw, mailcore.FolderAdaInbox, mailcore.AcctAda)
	if err != nil {
		t.Fatal(err)
	}
	...
	if strings.Contains(mailcore.SanitizeHTML(html), "script") || strings.Contains(mailcore.SanitizeHTML(html), "onclick") {
		t.Fatalf("sanitize %q", mailcore.SanitizeHTML(html))
	}
	if !strings.Contains(mailcore.HTMLToText(html), "Hi") {
		t.Fatal(mailcore.HTMLToText(html))
	}
	built := mailcore.BuildRFC822(mailcore.Message{To: "kai@example.com", Subject: "x", Body: "y"}, mailcore.Identity{Name: "Ada", Address: "ada@example.com", Signature: "sig"}, nil)
...
func TestGetMessageUsesDiskRawWithoutNetwork(t *testing.T) {
	dir := t.TempDir()
	cfg := mailcore.MailConfig{Accounts: []mailcore.AccountConfig{{
		ID: "home", Name: "Ada", Address: "ada@example.com",
		Identities: []mailcore.Identity{{ID: "home-1", Name: "Ada", Address: "ada@example.com", Default: true}},
	}}}
	s, err := mailcore.NewLocalStoreDir(cfg, dir)
	if err != nil {
		t.Fatal(err)
	}
	...
	id, err := s.Append(inbox.ID, mailcore.Message{Subject: "Cached", From: "kai@example.com", To: "ada@example.com", Body: "hello from disk raw"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLocalStoreDiskRoundTrip(t *testing.T) {
	...
	_, err = s.PutTag(mailcore.Tag{Name: "Work", Color: "#d35400"})
	if err != nil {
		t.Fatal(err)
	}
	s2, err := mailcore.NewLocalStoreDir(cfg, dir)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDisplayBodyPrefersPlainAndStripsHTML(t *testing.T) {
	plain := mailcore.DisplayBody(mailcore.Message{Body: "hello", HTML: "<b>nope</b>"})
	if plain != "hello" {
		t.Fatalf("prefer plain %q", plain)
	}
	only := mailcore.DisplayBody(mailcore.Message{HTML: `<p>Hi</p><script>alert(1)</script>`})
	if strings.Contains(only, "<") || strings.Contains(only, "script") || !strings.Contains(only, "Hi") {
		t.Fatalf("strip html %q", only)
	}
	tagged := mailcore.DisplayBody(mailcore.Message{Body: "<div>Only HTML</div>"})
...
func TestOpenStoreEmptyByDefault(t *testing.T) {
	...
	t.Setenv(mailcore.EnvMail, "")
	t.Setenv(mailcore.EnvHost, "")
	t.Setenv(mailcore.EnvUser, "")
	t.Setenv(mailcore.EnvConfig, filepath.Join(dir, "no-such-mail.json"))
	t.Setenv(mailcore.EnvData, filepath.Join(dir, "data"))
	s, err := mailcore.OpenStore()
	if err != nil {
		t.Fatal(err)
	}
	if s.Backend() != "imap" {
...
func TestOpenStoreMemoryStillDemo(t *testing.T) {
	t.Setenv(mailcore.EnvMail, "memory")
	s, err := mailcore.OpenStore()
	if err != nil {
		t.Fatal(err)
	}
...
func TestPutAccountWritesPasswordMode0600(t *testing.T) {
	...
	t.Setenv(mailcore.EnvConfig, path)
	t.Setenv(mailcore.EnvData, filepath.Join(dir, "data"))
	s, err := mailcore.NewLocalStoreDir(mailcore.MailConfig{}, filepath.Join(dir, "data"))
	if err != nil {
		t.Fatal(err)
	}
	acct, err := s.PutAccount(mailcore.AccountConfig{
		Name: "Ada", Address: "ada@example.com",
		IMAP: mailcore.ServerConfig{Host: "imap.example.com:993", User: "ada@example.com", Pass: "super secret password!!"},
	...
	if file.Accounts[0].Protocol != mailcore.ProtoIMAP {
...
func TestServerConfigPasswordPrefersInline(t *testing.T) {
	t.Setenv(mailcore.EnvPass, "from-env")
	if got := (mailcore.ServerConfig{}).Password(); got != "from-env" {
		t.Fatalf("default env %q", got)
	}
	if got := (mailcore.ServerConfig{Pass: "inline"}).Password(); got != "inline" {
		t.Fatalf("inline %q", got)
	}
	t.Setenv("UITK_MAIL_CUSTOM_PASS", "custom")
	if got := (mailcore.ServerConfig{PassEnv: "UITK_MAIL_CUSTOM_PASS"}).Password(); got != "custom" {
...
func TestRPCPutAccountAndEmptyFirstRun(t *testing.T) {
	sock, stop, err := mailcore.StartEmpty(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	defer stop()
	...
	got, err := cli.PutAccount(mailcore.AccountConfig{
		Name: "Ada", Address: "ada@example.com",
		IMAP: mailcore.ServerConfig{Host: "imap.example.com:993"},
	})
	if err != nil || got.ID == "" {
		t.Fatal(err, got)
	}
...
func TestRemoveAccountMenuAndFirstRunAgain(t *testing.T) {
	...
	defer cli.Close()
	if _, err := cli.PutAccount(mailcore.AccountConfig{
		Name: "Ada", Address: "ada@example.com",
		IMAP: mailcore.ServerConfig{Host: "imap.example.com:993"},
	}); err != nil {
		t.Fatal(err)
...
func TestConfigFromEnv(t *testing.T) {
	t.Setenv(mailcore.EnvHost, "imap.example.com:993")
	t.Setenv(mailcore.EnvUser, "you@example.com")
	t.Setenv(mailcore.EnvName, "You")
	cfg, err := mailcore.ConfigFromEnv()
