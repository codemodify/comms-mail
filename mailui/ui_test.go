...
func TestSelectAccountKeepsInboxList(t *testing.T) {
	...
	for _, n := range tree.Roots {
		...
		if id, ok := n.Data.(string); ok && id != "" && id != mailcore.AccountTags {
			acct = n
			break
		}
	}
...
func TestHiddenFromFolderTree(t *testing.T) {
	if !mailcore.HiddenFromFolderTree(mailcore.FolderUnifiedInbox) || !mailcore.HiddenFromFolderTree(mailcore.FolderUnifiedUnread) {
		t.Fatal("unified")
	}
	if !mailcore.HiddenFromFolderTree(mailcore.FolderCatPrimary) || !mailcore.HiddenFromFolderTree(mailcore.FolderID("smart/sf-invoices")) {
		t.Fatal("cat/smart")
	}
	if !mailcore.HiddenFromFolderTree(mailcore.FolderVIP) {
		t.Fatal("VIP folder should be hidden from the tree")
	}
	if mailcore.HiddenFromFolderTree(mailcore.FolderOutbox) || mailcore.HiddenFromFolderTree(mailcore.TagFolderID("Work")) {
		...
		case *widgets.TreeView:
			...
			walk = func(nodes []*widgets.TreeNode) {
				for _, n := range nodes {
					...
					if id, ok := n.Data.(mailcore.FolderID); ok && id == mailcore.FolderVIP {
						vipNode = true
					}
...
func assertTagsTree(t *testing.T, tree *widgets.TreeView) {
	...
	for _, n := range tree.Roots {
		...
		if n.Label == "Tags" || n.Data == mailcore.AccountTags {
			filters = n
		}
	}
	...
	want := mailcore.TagNames(mailcore.DefaultTags())
...
func TestAttachFileNameHelpers(t *testing.T) {
	if got := mailcore.AttachFileName("../x/mail-shortcuts.txt"); got != "mail-shortcuts.txt" {
		t.Fatalf("base %q", got)
	}
	if got := mailcore.AttachFileName(".."); got != "attachment" {
...
func TestOrderFolderChildrenOutboxLast(t *testing.T) {
	got := orderFolderChildren([]mailcore.Folder{
		{Name: "Outbox", ID: mailcore.FolderOutbox, Kind: mailcore.FolderCustom},
		{Name: "Projects", Kind: mailcore.FolderCustom},
		{Name: "Inbox", Kind: mailcore.FolderInbox},
		{Name: "Trash", Kind: mailcore.FolderTrash},
		{Name: "Junk", Kind: mailcore.FolderJunk},
		{Name: "Archive", Kind: mailcore.FolderArchive},
		{Name: "Drafts", Kind: mailcore.FolderDrafts},
		{Name: "Sent", Kind: mailcore.FolderSent},
	})
	...
	named := orderFolderChildren([]mailcore.Folder{
		{Name: "Outbox", Kind: mailcore.FolderCustom},
		{Name: "Inbox", Kind: mailcore.FolderInbox},
		{Name: "Notes", Kind: mailcore.FolderCustom},
	})
...
func treeHasOutboxRoot(tv *widgets.TreeView) bool {
	...
	for _, n := range tv.Roots {
		...
		if id, ok := n.Data.(mailcore.FolderID); ok && id == mailcore.FolderOutbox {
			return true
		}
...
func TestMailPreferencesTabs(t *testing.T) {
	...
	want := mailcore.TagNames(mailcore.DefaultTags())
	gotNames := mailcore.TagNames(prefs)
