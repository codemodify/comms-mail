...
func TestNewMailGoesThroughTheNotifier(t *testing.T) {
	rec := &recNotifier{}
	s := &session{notes: rec}
	s.onDaemonEvent(mailcore.Event{Method: mailcore.EventNotify, Title: "Ada Lovelace", Body: "Notes on the engine"})
	s.onDaemonEvent(mailcore.Event{Method: mailcore.EventNotify, Body: "2 new message(s)"})
	if len(rec.sent) != 2 {
