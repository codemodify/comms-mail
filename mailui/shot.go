...
func writeMailFolderShot(cli *mailcore.Client, path string, folder mailcore.FolderID) error {
	...
	widget.Walk(root, func(c widget.Component) {
		if tv, ok := c.(*widgets.TreeView); ok {
			...
			walk = func(nodes []*widgets.TreeNode) {
				for _, n := range nodes {
					if id, ok := n.Data.(mailcore.FolderID); ok && id == folder {
						hit = n
					}
					walk(n.Children)
				}
...
func writeMailShot(cli *mailcore.Client, path string, light bool, layout LayoutMode, cards bool, dens style.Density, menu int) error {
...
func writeComposeShot(cli *mailcore.Client, path string) error {
	a := uitoolkit.New(uitoolkit.Options{Look: style.DarkLook(), Headless: true})
...
func writePrefsShot(cli *mailcore.Client, path string, tab int) error {
	a := uitoolkit.New(uitoolkit.Options{Look: style.DarkLook(), Headless: true})
