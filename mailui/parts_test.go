...
func TestMIMEPartIDsAreUniqueAndNested(t *testing.T) {
	m, err := mailcore.ParseRFC822([]byte(nestedMIME), "f", "a")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPartFromRawReturnsAttachmentBytes(t *testing.T) {
	p, ok := mailcore.PartFromRaw([]byte(nestedMIME), "2")
	if !ok {
	...
	txt, ok := mailcore.PartFromRaw([]byte(nestedMIME), "1.1")
...
func TestLocalStoreGetPartDecodesAttachmentFromCachedRaw(t *testing.T) {
	...
	st.Folders = append(st.Folders, mailcore.Folder{ID: "home/inbox", AccountID: "home", Name: "Inbox", Kind: mailcore.FolderInbox})
	m := mailcore.Message{ID: "home/inbox:5", Folder: "home/inbox", AccountID: "home", UID: 5}
...
func TestGetPartRejectsIllegalPartID(t *testing.T) {
	...
	st.Folders = append(st.Folders, mailcore.Folder{ID: "home/inbox", AccountID: "home", Kind: mailcore.FolderInbox})
	m := mailcore.Message{ID: "home/inbox:5", Folder: "home/inbox", AccountID: "home", UID: 5}
	st.Messages = append(st.Messages, m)
	if _, err := st.GetPart(m.ID, "1] UID SEARCH ALL\r\nX"); err == nil {
		t.Fatal("a part id that could be injected into BODY.PEEK[] must be rejected")
	}
	if !mailcore.ValidPartID("1.2") || !mailcore.ValidPartID("2") || !mailcore.ValidPartID("1.2.TEXT") {
		t.Fatal("legitimate section ids must pass")
	}
	if mailcore.ValidPartID("../x") || mailcore.ValidPartID("1 2") {
...
func TestOpenPartRefusesExecutableAttachments(t *testing.T) {
	...
	st.Folders = append(st.Folders, mailcore.Folder{ID: "home/inbox", AccountID: "home", Kind: mailcore.FolderInbox})
	m := mailcore.Message{ID: "home/inbox:1", Folder: "home/inbox", AccountID: "home"}
...
func TestHTMLToTextDecodesEntitiesAndKeepsBareLessThan(t *testing.T) {
	...
	for in, want := range cases {
		if got := mailcore.HTMLToText(in); got != want {
...
func TestQuotedPrintableAndCharsetDecoding(t *testing.T) {
	...
	m, err := mailcore.ParseRFC822([]byte(raw), "f", "a")
	if err != nil {
		t.Fatal(err)
	}
	if m.Subject != "Grüße" {
...
func TestIMAPUTF7RoundTrip(t *testing.T) {
	...
	for utf8Name, wire := range cases {
		if got := mailcore.EncodeIMAPUTF7(utf8Name); got != wire {
			t.Fatalf("encode(%q) = %q, want %q", utf8Name, got, wire)
		}
		if got := mailcore.DecodeIMAPUTF7(wire); got != utf8Name {
...
func TestOfflineSendKeepsAttachmentsInTheQueue(t *testing.T) {
	dir := t.TempDir()
	t.Setenv(mailcore.EnvConfig, filepath.Join(dir, "mail.json"))
	st, err := mailcore.NewLocalStoreDir(mailcore.MailConfig{Accounts: []mailcore.AccountConfig{{
		ID: "home", Address: "ada@example.com",
		IMAP: mailcore.ServerConfig{Host: "imap.example.com:993", User: "ada@example.com", Pass: "p"},
		SMTP: mailcore.ServerConfig{Host: "smtp.example.com:587", User: "ada@example.com", Pass: "p"},
	}}}, dir)
	if err != nil {
		t.Fatal(err)
	}
	st.SetOnline(false)
	files := []mailcore.AttachedFile{{Name: "notes.txt", MIME: "text/plain", Data: []byte("hello")}}
	if _, err := st.SendViaSMTP("home", "", mailcore.Message{
	...
	st2, err := mailcore.NewLocalStoreDir(mailcore.MailConfig{}, dir)
	if err != nil {
		t.Fatal(err)
	}
	ops2 := st2.ListOutbox()
...
func TestFailedSendQueuesOnceNotTwice(t *testing.T) {
	...
	st, err := mailcore.NewLocalStoreDir(mailcore.MailConfig{Accounts: []mailcore.AccountConfig{{
		...
		SMTP: mailcore.ServerConfig{Host: "127.0.0.1:1", User: "ada@example.com", Pass: "p", TLSMode: string(mailcore.TLSPlain)},
	}}}, dir)
	if err != nil {
		t.Fatal(err)
	}
	msg := mailcore.Message{From: "ada@example.com", To: "bob@example.com", Subject: "s", Body: "b"}
...
func TestStoreEmitsChangeEventsForBackgroundWork(t *testing.T) {
	...
	got := make(chan mailcore.StoreEvent, 4)
	st.SetOnChange(func(ev mailcore.StoreEvent) { got <- ev })
	st.Emit(mailcore.StoreEvent{Reason: "push", AccountID: "home", Count: 2})
	select {
	case ev := <-got:
	...
	st.SetOnChange(nil)
	st.Emit(mailcore.StoreEvent{Reason: "push"})
	select {
	case ev := <-got:
...
func TestDaemonBroadcastsChangeOnBackgroundSync(t *testing.T) {
	...
	go func() { done <- mailcore.ListenAndServe(ctx, sock, store) }()
	...
	defer cli.Close()
	events := make(chan mailcore.Event, 8)
	cli.OnEvent(func(ev mailcore.Event) { events <- ev })

	if _, err := cli.PutTag(mailcore.Tag{Name: "Zeta", Color: "#ff0000"}); err != nil {
		t.Fatal(err)
	}
	...
	for {
		select {
		case ev := <-events:
			if ev.Method == mailcore.EventChanged {
				cancel()
				<-done
				return
			}
...
func TestSessionRefreshesOnDaemonEvent(t *testing.T) {
	...
	s.onDaemonEvent(mailcore.Event{Method: mailcore.EventChanged})
...
func TestUnreadAllIsOneRoundTrip(t *testing.T) {
	...
	go func() { done <- mailcore.ListenAndServe(ctx, sock, mailcore.NewDemoStore()) }()
	waitForSocket(t, sock)
	cli, err := mailcore.DialWait(sock, 2*time.Second)
	if err != nil {
		t.Fatal(err)
...
func TestReadAttachmentsShipsBytes(t *testing.T) {
	...
	files, err := mailcore.ReadAttachments([]string{path})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := mailcore.ReadAttachments([]string{filepath.Join(dir, "missing")}); err == nil {
...
func TestDaemonDoesNotReadArbitraryPaths(t *testing.T) {
	...
	raw := mustMarshal(t, mailcore.ComposeParams{AccountID: "a", Message: mailcore.Message{To: "x@y.z"}})
...
func TestSelectionIsPrunedWhenRowsVanish(t *testing.T) {
	s := &session{
		rows:     []mailcore.Message{{ID: "a"}, {ID: "b"}},
		selected: []mailcore.MessageID{"a", "gone", "b"},
	}
	s.pruneSelection()
...
// waitForSocket and contextForTest mirror the daemon-side test helpers:
// the UI tests start a real daemon of their own, and a test helper cannot
